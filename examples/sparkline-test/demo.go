...
func DemoSparkline() {
	...
	for i := 0; i < 15; i++ {
		value := 20 + 50*math.Sin(float64(i)*0.3) + r.Float64()*10
		s.AddPoint(value)
...
func generateRandomData(count int, minVal, maxVal float64) []float64 {
	data := make([]float64, count)
	for i := range data {
		data[i] = minVal + r.Float64()*(maxVal-minVal)
	}
	return data
}
...
func generateTrend(count int, start, slope, noise float64) []float64 {
	data := make([]float64, count)
	for i := range data {
		data[i] = start + slope*float64(i) + (r.Float64()-0.5)*noise
	}
	return data
}
...
func generateSpikes(count int, base float64, spikeProb, spikeHeight float64) []float64 {
	...
	for i := range data {
		if r.Float64() < spikeProb {
			data[i] = base + spikeHeight*(r.Float64()-0.5)
		} else {
			data[i] = base + (r.Float64()-0.5)*5
		}
	}
	return data
}
