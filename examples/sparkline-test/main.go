...
func (t *TrendGenerator) Next() float64 {
	value := t.base + t.trend*t.time + t.noise*(t.rng.Float64()-0.5)
	t.time += 0.1
	return value
}
...
func (s *SpikeGenerator) Next() float64 {
	if s.rng.Float64() < s.spikeProb {
		return s.base + s.spikeHeight*(s.rng.Float64()-0.5)
	}
	return s.base + (s.rng.Float64()-0.5)*0.1
}
...
func initialModel() Model {
	...
	configs := []struct {
		...
		{
			title:     "CPU Usage (%)",
			generator: &RandomGenerator{min: 0, max: 100, rng: rng},
		...
		},
		{
			...
			generator: &TrendGenerator{base: 10, trend: 0.1, noise: 2, rng: rng},
			style:     sparkline.StyleLine,
		},
		{
			title:     "Disk Activity",
			generator: &SpikeGenerator{base: 20, spikeProb: 0.1, spikeHeight: 50, rng: rng},
...
including bars, dots, lines, and filled areas.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Initialize random seed (removed - now done in initialModel)
