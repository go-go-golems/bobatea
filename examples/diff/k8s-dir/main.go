...
func buildItem(rel string, b, a map[string]any, sens map[string]struct{}) diff.DiffItem {
	...
	for _, r := range rm {
		_, s := sens[r.Path]
		cs = append(cs, chg{path: r.Path, status: diff.ChangeStatusRemoved, before: r.BeforeAny, sensitive: s})
	}
	for _, adx := range ad {
		_, s := sens[adx.Path]
		cs = append(cs, chg{path: adx.Path, status: diff.ChangeStatusAdded, after: adx.AfterAny, sensitive: s})
	}
	for _, u := range up {
		_, s := sens[u.Path]
		cs = append(cs, chg{path: u.Path, status: diff.ChangeStatusUpdated, before: u.BeforeAny, after: u.AfterAny, sensitive: s})
	}
...
func main() {
	...
	b, err := collectYAML(beforeDir)
	if err != nil {
		log.Fatal(err)
	}
	a, err := collectYAML(afterDir)
	if err != nil {
		log.Fatal(err)
	}
	...
	cfg.Title = "Kubernetes Manifests Diff"
