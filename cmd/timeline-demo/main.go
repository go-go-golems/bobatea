...
type updateText struct{ id timeline.EntityID; text string; version int64 }
type finishText struct{ id timeline.EntityID; text string }

type startTools struct{ id timeline.EntityID }
type addTool struct{ id timeline.EntityID; call map[string]any; version int64 }
...
func (m demoModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.KeyMsg:
        ...
        case "o":
            ...
            return m, tea.Batch(
                tea.Tick(300*time.Millisecond, func(time.Time) tea.Msg { return addTool{id: id, version: 1, call: map[string]any{"id": "a", "name": "search", "status": "pending"}} }),
                tea.Tick(900*time.Millisecond, func(time.Time) tea.Msg { return addTool{id: id, version: 2, call: map[string]any{"id": "a", "name": "search", "status": "done", "result": "ok"}} }),
                tea.Tick(1300*time.Millisecond, func(time.Time) tea.Msg { return addTool{id: id, version: 3, call: map[string]any{"id": "b", "name": "write", "status": "pending"}} }),
                tea.Tick(1800*time.Millisecond, func(time.Time) tea.Msg { return addTool{id: id, version: 4, call: map[string]any{"id": "b", "name": "write", "status": "done", "result": "saved"}} }),
            )
        }
    case updateText:
