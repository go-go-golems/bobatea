...
func (d *dispatcher) TryHandle(ctx context.Context, input string, emit Emitter) bool {
    ...
    if parsed.Name == "" {
        // suggest commands
        var names []string
        for _, c := range d.reg.List() { names = append(names, c.Name) }
        sort.Strings(names)
        md := "Available commands:\n\n"
        for _, n := range names { md += fmt.Sprintf("- /%s\n", n) }
...
func (d *dispatcher) Complete(ctx context.Context, raw string, caret int) ([]autocomplete.Suggestion, CompletionState, error) {
    ...
    if st.Phase == PhaseName {
        ...
        partial := st.Partial
        if strings.HasPrefix(partial, "/") { partial = strings.TrimPrefix(partial, "/") }
        if partial == "" { partial = st.Name }
