...
func parseInput(raw string) (Input, []token) {
    ...
    if len(toks) == 0 { return out, toks }
    if !strings.HasPrefix(toks[0].text, "/") { return out, toks }
...
func completionState(raw string, caret int) (CompletionState, []token) {
    ...
    st.Parsed = in
    if caret < 0 || caret > len(raw) { caret = len(raw) }
    ...
    if st.PositionalIndex < 0 { st.PositionalIndex = 0 }
    return st, toks
}
