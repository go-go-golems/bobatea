...
func (s *SlashBackend) GetBySlug(ctx context.Context, slug string) (*replhelp.Section, error) {
    ...
    if name == "" { return nil, fmt.Errorf("not found") }
...
func buildCommandContent(c *slash.Command) string {
    ...
    if strings.TrimSpace(c.Summary) != "" { b.WriteString(" — "); b.WriteString(c.Summary) }
    b.WriteString("\n\n")
    ...
    if len(c.Schema.Positionals) > 0 || len(c.Schema.Flags) > 0 {
        ...
        if len(c.Schema.Positionals) > 0 {
            ...
            for _, a := range c.Schema.Positionals {
                ...
                if a.Required { b.WriteString(" (required)") }
                if a.Variadic { b.WriteString(" (variadic)") }
                if strings.TrimSpace(a.Description) != "" { b.WriteString(": "); b.WriteString(a.Description) }
                b.WriteString("\n")
            }
            b.WriteString("\n")
        }
        if len(c.Schema.Flags) > 0 {
            ...
            for _, f := range c.Schema.Flags {
                ...
                if strings.TrimSpace(f.Description) != "" { b.WriteString(": "); b.WriteString(f.Description) }
                b.WriteString("\n")
            }
            b.WriteString("\n")
        }
