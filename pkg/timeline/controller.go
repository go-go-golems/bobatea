...
func (c *Controller) OnDeleted(e UIEntityDeleted) {
    ...
    c.store.remove(e.ID)
    c.cache.invalidateByID(e.ID)
    if c.selected >= len(c.store.order) { c.selected = len(c.store.order)-1 }
}

func (c *Controller) SelectNext() { if c.selected+1 < len(c.store.order) { c.selected++ } }
func (c *Controller) SelectPrev() { if c.selected > 0 { c.selected-- } }
...
func (c *Controller) pickRenderer(rec *entityRecord) Renderer {
    if r, ok := c.reg.GetByKey(rec.Renderer.Key); ok { return r }
    if r, ok := c.reg.GetByKind(rec.Renderer.Kind); ok { return r }
