...
// Tool-specific renderers (demo-ready) --------------------------------------
...
func (r *ToolWeatherRenderer) Render(props map[string]any, width int, theme string) (string, int, error) {
    ...
    log.Debug().Str("component", "timeline_renderer").Str("renderer", r.Key()).Int("width", width).Int("len", len(status)).Msg("rendered")
    return status, 1, nil
}
...
func (r *ToolWebSearchRenderer) RelevantPropsHash(props map[string]any) string { return hashMap(props) }
