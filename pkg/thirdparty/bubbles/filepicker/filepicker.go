...
func (m Model) enterDir(dir string) (Model, tea.Cmd) {
	...
	if isSymlink {
		symlinkPath, _ := filepath.EvalSymlinks(dir)
...
func (m Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case key.Matches(msg, m.KeyMap.Open):
			...
			if isDir {
				dir := filepath.Join(m.CurrentDirectory, f.Name())
				return m.enterDir(dir)
			} else {
				if isSymlink {
